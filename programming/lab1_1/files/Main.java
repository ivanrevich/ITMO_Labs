public class Main {
    ...
    public static double getNewW1Elem(float xl, long wl){
        if(wl==19){
            return Math.pow(0.5*Math.atan((xl+0.5)/5), 3);
        }
        else if(checkSecondStatement(wl)){
            return Math.pow(Math.pow(Math.E, Math.pow(xl, ((double) 1 /3)))*(((double) 1 /2)+Math.tan(Math.pow(xl, ((double) 1 /3)))), Math.pow(Math.pow(Math.E, xl), Math.pow(Math.E, xl)-1));
        }
        else{
            return Math.atan(Math.cos( Math.pow( ((double) (1/2))*Math.sin( Math.pow(3*xl, 2) ), 2) ));
        }
    }
    ...
    public static void outputResult(double[][] wl){
        for(int i=0; i<wl.length; i++) {
            for (int j = 0; j < wl[i].length; j++) {
                if(j!=wl[i].length-1) System.out.format("%9.2e ", wl[i][j]);
                else System.out.format("%9.2e", wl[i][j]);
            }
    ...
    public static void main(String[] args) {
        ...
        for(int i=0; i<15; i++) {
            x[i] = -2.0f + (float) (Math.random() * ((3.0f-(-2.0f))+1.0f));
        }
        for(int i=0; i<17; i++){
            for (int j=0; j<15; j++){
